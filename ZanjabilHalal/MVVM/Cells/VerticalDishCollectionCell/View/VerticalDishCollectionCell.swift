//
//  VerticalDishCollectionCell.swift
//  ZanjabilHalal
//
...
final class VerticalDishCollectionCell: UICollectionViewCell, LoadNidoble, MVVMViewProtocol {
    ...
    private func setup(){
        self.mainView.cornerRadius(60, true)
        self.previewImageView.cornerRadius(52, true)
    }
    ...
    // MARK: - create cell
    static func create(with collection: UICollectionView, with indexPath: IndexPath) -> VerticalDishCollectionCell {
        let nib = UINib(nibName: String(describing: Self.self), bundle: nil)
        collection.register(nib, forCellWithReuseIdentifier: String(describing: Self.self))
        let cell = collection.dequeueReusableCell(withReuseIdentifier: String(describing: Self.self), for: indexPath) as! VerticalDishCollectionCell
        return cell
    }
}
